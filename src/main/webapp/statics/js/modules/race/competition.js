...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.competition.id == null ? "competition/save" : "competition/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.competition),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "competition/delete",
				    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "competition/info/"+id, function(r){
                vm.competition = r.competition;
            });
		},
